...
def upgrade() -> None:
    ...
    #
    # Renumbered from 076 to 078 on this branch: 076/077 (payee original name
    # and the tax-id backfill) were already applied in production before this
    # landed, so this chains after them rather than renumbering what is
    # already stamped in `alembic_version`.
    op.execute(
        """
        CREATE TEMP TABLE payee_dedupe_078 ON COMMIT DROP AS
    ...
    op.execute(
        ...
        FROM (
            ...
            FROM payee_dedupe_078 duplicates
            JOIN payees
    ...
    op.execute(
        ...
        WITH candidates AS (
            ...
            JOIN payee_dedupe_078 duplicates
    ...
    op.execute(
        ...
        FROM (
            ...
            LEFT JOIN payee_dedupe_078 duplicates
    ...
    op.execute(
        ...
        BEGIN
            FOR duplicate IN SELECT * FROM payee_dedupe_078 LOOP
                UPDATE rules
                SET conditions = (
                    SELECT jsonb_agg(
                        CASE
    ...
    op.execute(
        """
        DELETE FROM payees
        USING payee_dedupe_078 duplicates
