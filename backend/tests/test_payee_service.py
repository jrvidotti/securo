...
async def test_get_or_create_payee_returns_case_insensitive_concurrent_winner(
    ...
    # Every lookup that runs before the insert has to miss, or the race never
    # happens. Resolution tries the display name and then the original name,
    # and skips the document lookup because this call carries no document.
    lookups_before_insert = 2

    async def miss_until_insert(*args, **kwargs):
        nonlocal lookups_before_insert
        if lookups_before_insert > 0:
            lookups_before_insert -= 1
            return missing
        return await real_execute(*args, **kwargs)

    monkeypatch.setattr(session, "execute", miss_until_insert)
